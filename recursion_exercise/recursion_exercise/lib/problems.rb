...
def sum_array(array)
    if array.empty?
        ...
        return array[0] + sum_array(array[1...array.length])
    end
end
...
def flatten(data)
    ...
    if !data.kind_of?(Array)
        ...
        return flatten(data[0]) #I need this to remove a nested array
    else
        return flatten(data[0])+ flatten(data[1...data.length])
    end
end